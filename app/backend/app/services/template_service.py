...
def create_template(db: Session, payload: TemplateCreate) -> tuple[ScheduleTemplate, int]:
    ...
    db.commit()
    db.refresh(t)
    created = lesson_service.materialize_template(db, t)
    return t, created
...
def update_template(
    ...
    if apply_mode == "update_all":
        from_date = today()
        affected = lesson_service.regenerate_template_future(db, t, from_date=from_date)
        return t, affected

    db.commit()
    db.refresh(t)

    if apply_mode == "from_date" and apply_from:
        from_date = apply_from
    else:
        from_date = today()
