...
async function save() {
  ...
  try {
    ...
    settingsError.value = parseFormError(error, '设置保存失败，已恢复服务器中的设置')
...
<template>
  <div class="grid gap-4">
    <section class="glass p-4 md:p-5">
      ...
      <form data-form="settings" class="mt-4 grid gap-3.5 md:grid-cols-2" @submit.prevent="save">
        ...
        <div class="md:col-span-2"><AsyncButton data-action="save-settings" :pending="saving" pending-label="保存中…">保存设置</AsyncButton></div>
      </form>
    </section>
  ...
  <ConfirmDialog v-model:open="restoreDialogOpen" title="覆盖当前数据？" description="恢复备份会覆盖当前数据。操作前系统会自动保存旧数据库副本，但仍建议先下载最新备份。" confirm-label="确认覆盖并恢复" :pending="restoring" pending-label="恢复中…" @confirm="restoreBackup" />
</template>
